...
(
  ...
  default charset = utf8mb4 comment ='火车车站列表';
...
(
    ...
    `date`               date          not null comment '日期',
    `train_code`         varchar(20)   not null comment '车次编号',
    `departure`          varchar(20)   not null comment '出发站',
    `departure_pinyin`   varchar(50)   not null comment '出发站拼音',
    `departure_time`     time          not null comment '出发时间',
    `departure_index`    int           not null comment '出发站序|本站是整个车次的第几站',
    `destination`        varchar(20)   not null comment '到达站',
    `destination_pinyin` varchar(50)   not null comment '到达站拼音',
    `arrival_time`       time          not null comment '到站时间',
    `arrival_index`      int           not null comment '到站站序|本站是整个车次的第几站',
    `first_class`        int           not null comment '一等座余票',
    `first_class_price`  decimal(8, 2) not null comment '一等座票价',
    `second_class`       int           not null comment '二等座余票',
    `second_class_price` decimal(8, 2) not null comment '二等座票价',
    `soft_sleeper`       int           not null comment '软卧余票',
    `soft_sleeper_price` decimal(8, 2) not null comment '软卧票价',
    `hard_sleeper`       int           not null comment '硬卧余票',
    `hard_sleeper_price` decimal(8, 2) not null comment '硬卧票价',
    `create_time`        datetime(3) comment '新增时间',
    `update_time`        datetime(3) comment '修改时间',
    primary key (`id`),
    unique key `date_train_code_departure_destination_unique` (`date`, `train_code`, `departure`, `destination`)
) engine = innodb
